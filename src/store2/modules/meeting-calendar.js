...
import axios, {MEETING_CALENDAR, turnObjectToQueryString} from "../api";
...
export default{
    ...
    actions:{
        listSocietyMemberWhichHasMeetingToday({ commit }, {query={}, reload=false}={}){
            const url = MEETING_CALENDAR.index + "?" + turnObjectToQueryString(query);
            debug("url", url);
            ...
            return axios.get(MEETING_CALENDAR.index)
            .then(resp=>{
                //debug("resp", resp);
        ...
        saveMeetingCalendar({ commit }, {society_id=null, year=null, meeting_dates=[]} = {}){
            ...
            commit(setIsLoading, true)
            return axios.put(
                MEETING_CALENDAR.save_meeting_calendar + `${society_id}/${year}`, 
                {meeting_dates}
            )
            .then(resp=>{
        ...
        getMeetingCalendarForSocietyWithinAyear({ commit }, {society_id=null, year=null, reload=false} = {}){
            ...
            if(!reload && cacheMeetingCalendarIDs){
                ...
                if(cacheMeetingCalendars){ 
                    return resolveWithDataFromStore( commit, cacheMeetingCalendars );
                }//end if
            }//end if
            commit(setIsLoading, true)
            return axios.get(
                MEETING_CALENDAR.get_society_meeting_calendar_for_year + `${society_id}/${year}`
            )
            .then(resp=>{
